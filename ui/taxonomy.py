...
#   DOC_CLASSIFICATION 이고, 화면은 거기서 내보낸 JSON 을 그대로 읽는다
...
#=> 화면이 쓰는 분류 체계는 MpowerV11 이 내보낸 JSON(doc_classification_export.json)에서
#   바로 읽는다(따로 떨어진 스냅샷 파일은 없다 — 2026-09-22).
#    1) nodes 의 dc_id·parent_dc_id·order_num·title·status 를 옮긴다
#    2) 각 노드의 전체경로("법무/규정 > 계약서")를 부모를 따라 올라가며 미리 만든다 —
...
#                    "by_id": {dc_id: {dc_id,parent,order,title,status,path}},
#                    "path": 읽은 파일 경로}
...
def load_from_export(path):
    ...
    # 부모가 목록에 없으면(꼬인 데이터) 거기서 멈춘다 — 무한 반복을 막으려고 방문한 노드를 기억한다.
...
#   항상 경로를 함께 보여준다").
#
# -in: tax   = load_from_export() 결과(None 이면 dc_id 를 그대로 돌려준다)
# -in: dc_id = 찾을 분류 아이디
#
...
#
# -in: tax = load_from_export() 결과(None 이면 빈 목록)
#
