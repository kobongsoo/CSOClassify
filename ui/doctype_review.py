...
def reason_block(cand):
    ...
    summary = None
    if len(parts) > 1:
        # 지금 엔진은 근거를 쌓아 올리는 방식(noisy-OR) 하나뿐이지만, 예전 채점
        # (legacy, 2026-09-22 제거)으로 만든 결과 파일이 남아 있을 수 있다 — 그래서
        # 결과를 보고 가린다. 최종 점수가 근거들의 '최댓값과 같으면' 가장 강한
        # 하나만 쓴 것(legacy), 그보다 크면 근거가 쌓여 올라간 것(noisy-OR).
