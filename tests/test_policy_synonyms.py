...
#------------------------------------------------------------------
# 매뉴얼 계열 이름은 실제로 쓰는 말만 받는다 (2026-09-15)
#=> 예전 꼬리말 '매뉴얼 → 사용설명서·안내서' 는 앞말에 붙어 '운영사용설명서' 같은
#   아무도 안 쓰는 말을 만들었다. 지금은 표기 변형(메뉴얼)과 '가이드' 로 바꾼다.
#    1) 사전에 없는 'OO매뉴얼' 이 'OO메뉴얼'·'OO가이드' 를 받는지 본다
#    2) 옛 치환이 만들던 말이 더는 안 나오는지 본다
#    3) '설명서' 꼬리말이 없어서 '사업설명서' 가 '사업매뉴얼' 로 번지지 않는지 본다
#
# -in: 없음
#
# -out: 없음
# -out: error = 기대와 다르면 AssertionError, 사전이 없으면 skip
#------------------------------------------------------------------
def test_매뉴얼_계열은_실제_쓰는_말만_받는다():
    syn = real_syn()
    # 사전에 없는 이름은 꼬리말 그물로 표기 변형과 '가이드' 를 받아야 한다
    업무 = DRE.synonyms_of("업무매뉴얼", syn)
    assert "업무메뉴얼" in 업무 and "업무가이드" in 업무
    assert "업무사용설명서" not in 업무 and "업무안내서" not in 업무
    assert "사용자매뉴얼" in DRE.synonyms_of("사용자가이드", syn)
    # 별칭으로 적은 하위 이름에 넓은 말(안내문·사용법)이 따라오면 안 된다
    운영 = DRE.synonyms_of("운영매뉴얼", syn)
    assert "운영메뉴얼" in 운영 and "운영사용설명서" not in 운영
    assert not {"안내문", "사용법", "가이드"} & set(운영)
    # 투자·사업설명서는 매뉴얼이 아니다 — '설명서' 꼬리말을 두지 않는다
    assert not any("매뉴얼" in w for w in DRE.synonyms_of("사업설명서", syn))
