...
def build_parser():
    ...
    p.add_argument("--idle-timeout", type=int, default=config.DEFAULT_IDLE_TIMEOUT, help="데몬 유휴 종료(초)")
...
#   --recursive 를 주면 하위 폴더까지 재귀로 훑는다(기본은 해당 폴더만).
#    1) --glob 의 콤마·중괄호를 여러 패턴으로 펼친다(다중 확장자 지원)
#    2) --recursive: <dir>/**/<glob> 을 recursive=True 로 확장(하위폴더 포함)
#    3) 기본       : <dir>/<glob> 만(하위폴더 제외)
#
# -in: args = argparse 결과(dir/glob/recursive)
#
...
def collect_files(args):
    ...
    if args.dir:
        ...
        for gp in expand_glob_patterns(args.glob):
            # --recursive 면 '**' 로 하위폴더까지, 아니면 해당 폴더만 매칭한다.
            if recursive:
                found.extend(glob.glob(os.path.join(args.dir, "**", gp), recursive=True))
            else:
                found.extend(glob.glob(os.path.join(args.dir, gp)))
...
def run_text_only(files, opts, out_fp):
    ...
    extractor = SynapExeExtractor()
    code = config.EXIT_OK
    for path in files:
        try:
            raw = extractor.extract(path, save_dir=opts.get("save_dir"))
            text = clean_text(raw)
            print(f"===== {path} =====", file=(out_fp or sys.stdout))
...
#   [전파까지] --auto-propagate 면 분류 직후 보류(none) 문서를 seed(--seeds 외부 +
#     내부 seed_eligible)로 전파해 구제한 뒤 출력한다(문서 1개 확인에도 유용). 이때
#     보류 문서 벡터가 필요하므로 임베딩이 꺼져 있으면 자동으로 --embed-needed 처럼 켠다.
#     레코드를 다 모은 뒤 한꺼번에 전파·출력하므로, 이 경우엔 스트리밍 대신 배치 출력.
#   [분류방식 배타옵션]
#       · --rule-only  : 규칙(cso_rules.yaml)만으로 분류. 임베딩·전파를 아예 안 한다(가장 빠름).
...
def run_classify(files, args, out_fp):
    ...
    from .extract.base import ExtractError
    from .clean import clean_text
    from .classify import load_rules, build_record, now_iso
    ...
    # 분류 방식 결정 — 배타 3분기: 벡터만 / 규칙만 / 기존(혼합).
    if vector_only:
        # 규칙 미사용: 전량 임베딩 후 seed 와 비교해 등급을 정한다 → 비교 기준 seed 파일 필수.
        if not args.seeds or not os.path.isfile(args.seeds):
            print("[csoclassify] --vector-only 는 비교 기준 seed 파일이 필요합니다: "
                  "--seeds <cso_seed.jsonl>", file=sys.stderr)
            return config.EXIT_ARG_ERROR
        embed_mode = "all"      # 모든 문서를 임베딩해야 seed 와 비교 가능
        auto_prop = True        # 전파(=벡터 비교)로 등급을 정하므로 항상 켠다
    elif rule_only:
        # 규칙만: 임베딩·전파를 하지 않는다(가장 빠름). 임베딩 관련 플래그가 있어도 무시.
        embed_mode = "none"
        auto_prop = False
    else:
        # 기존 동작: 임베딩 정책 needed(선택적) > all(전량) > none. embed_needed 가 우선.
    ...
    # 임베딩이 필요한 정책일 때만 임베더 준비(모델 로딩은 첫 ensure_loaded 에서 지연).
    embedder = None
    if embed_mode != "none":
        from .embed.onnx_embedder import OnnxEmbedder
        spec = config.get_model_spec(args.model)
        embedder = OnnxEmbedder(spec, num_threads=args.num_threads)
    ...
    # -out: error = 없음
    #--------------------------------------------------------------
    # 배치(--dir)면 json 을 유효한 배열로 감싸고, 단건(--file)이면 객체 하나로 낸다.
    writer = output.RecordWriter(out_fp or sys.stdout, args.fmt,
                                 multi=bool(getattr(args, "dir", None)))
    ...
    for i, path in enumerate(files, 1):
        try:
            ...
            raw = extractor.extract(path, save_dir=None)
            text = clean_text(raw)
        except ExtractError as e:
            print(f"[csoclassify] 추출 실패: {path} :: {e}", file=sys.stderr)
        ...
        if embedder is not None and need_vec:
            try:
                embedder.ensure_loaded()
                result, _ = embedder.embed_document(
                    text, args.max_tokens, args.overlap, normalize=True, per_chunk=False
                )
                # build_record(vector=...) 과 동일 형식으로 부착(등급은 이미 확정, 벡터만 추가).
                rec["vector"] = list(result)
                embedded += 1
    ...
    # 오탐률·분포를 한눈에 볼 수 있게 요약 한 줄(파이프 안전하게 stderr).
    print(
        f"[classify] files={len(files)} "
        f"C={counts.get('C', 0)} S={counts.get('S', 0)} O={counts.get('O', 0)} "
        f"none={counts.get('none', 0)} emb={embedded} rules={ruleset.version}",
        file=sys.stderr,
    )
    return code
...
def run_embed(files, args, opts, out_fp):
    ...
    def process_inprocess(path):
        if ip_extractor["v"] is None:
            ip_extractor["v"] = SynapExeExtractor()
    ...
    code = config.EXIT_OK
    totals = []
    # 배치(--dir)면 json 을 유효한 배열로 감싼다(단건 --file 은 객체 하나).
    writer = output.RecordWriter(out_fp or sys.stdout, args.fmt,
                                 multi=bool(getattr(args, "dir", None)))
    for path in files:
        try:
            if client is not None:
                ...
                try:
                    result = client.request_embed(path, opts)
                except DaemonUnavailable as e:
                    if args.verbose:
                        print(f"[csoclassify] 데몬 폴백(in-process): {e}", file=sys.stderr)
                    # 폴백은 파일 로그에 항상 남겨 원인을 나중에 확인할 수 있게 한다.
                    log.warning("데몬 사용 불가 → in-process 폴백 file=%s :: %s", path, e)
                    result = process_inprocess(path)
            else:
                result = process_inprocess(path)
            ...
            print(f"[csoclassify] 추출 실패: {path} :: {e}", file=sys.stderr)
            log.error("추출 실패 file=%s :: %s", path, e)
            code = config.EXIT_EXTRACT_FAIL
        except EmbedError as e:
            print(f"[csoclassify] 임베딩 실패: {path} :: {e}", file=sys.stderr)
            log.error("임베딩 실패 file=%s :: %s", path, e)
            code = config.EXIT_EMBED_FAIL
        except RuntimeError as e:
            # 데몬이 돌려준 처리 오류(추출/임베딩)를 코드로 환원.
            msg = str(e)
            print(f"[csoclassify] 처리 실패: {path} :: {msg}", file=sys.stderr)
            # 스택까지 파일 로그에 남겨 원인 추적을 돕는다.
            log.exception("처리 실패 file=%s :: %s", path, msg)
...
def main(argv=None):
    ...
    try:
        ...
        if args.text_only:
            return run_text_only(files, opts, out_fp)
        if args.embed:
            return run_embed(files, args, opts, out_fp)
        return run_classify(files, args, out_fp)
    finally:
        if out_fp is not None:
            out_fp.close()
