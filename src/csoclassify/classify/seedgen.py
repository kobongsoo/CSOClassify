...
#    0) 검토 대상 표시(review)가 없을 것 — 파일명 핵어로만 붙은 약한 라벨(13장 D1)
...
def candidate_ok(cand, taxonomy, t_seed, min_sources=DEFAULT_MIN_SOURCES):
    ...
    # 0) 검토 대상으로 표시된 약한 라벨(파일명 핵어로만 붙은 것)도 씨앗이 될 수
    #    없다(13장 D1 ③). 파일명만으로 붙은 라벨이 기준이 되면 틀린 라벨이
    #    임베딩을 타고 다른 문서로 번진다. 지금은 점수(t_low)가 t_seed 에 한참
    #    못 미쳐 어차피 걸리지만, 문턱을 낮추는 날 조용히 새어 들어가지 않게
    #    근거로도 막아 둔다.
    if cand.get("review"):
        return False, "검토대상"
